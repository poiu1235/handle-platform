...
// 会员卡的"显式无图标"哨兵（2026-09-06 用户裁定，对齐余额的无图标体验）：
// icon_key 存它 = 用户明确选择"无"——展示层渲染为名称行前的小菱形，且不再做
// 按名称自动匹配。与 null 区分：null = 未指定，继续按名称自动匹配。
// 取值刻意避开任何真实图标文件名（清单 key 是素材文件名，不会是 __none__）。
export const NO_ICON_KEY = '__none__'
