...
| 关联文档 | `design-language.md`（视觉与设计规范，v2 起并入会员页 cd- 体系与天数环）、`cards-db.md`（v3.2 库表与接口设计，与本文档同步）、`membership-user-manual.md`（功能操作手册）；`lifestyle-cardpack-prd_bak.md`、`lifestyle-cardpack-prd-v2_bak.md` 为历史版本，仅存档、不再维护 |
...
| 静默 | 按卡的提醒开关：不静默（提醒中）/ 本周期静默 / 永久静默；只关提醒，不影响状态与循环；以三态图标表达（remind=提醒中 / remind-disable=本周期 / close-remind=永久），点击循环切换（2026-09-03 裁定，3.3.2） |
...
- 静默状态必须**可见、可逆**（4-B22）。
- **触点交互（2026-09-03 裁定；图标素材名 2026-09-06 更新）**：静默操作统一为**三态图标点击循环**——remind.svg（提醒中）/ remind-disable.svg（本周期静默）/ close-remind.svg（永久静默）表达当前状态，点击切到下一状态：
...
flowchart TD
    ...
    B --> C["逐行解析与校验：格式 / 起始日∈[今天−2年,今天]（配置A）/ DDL∈[起始日,今天+2年]（配置B）/ 新增行续费开则必填周期（扣款日自动取DDL）"]
    C --> D{"有错误行?"}
...
- **B14 · 自动续费卡缺扣款日/周期** → 手工创建被表单强制（周期必选、扣款日自动 = DDL）；新增行导入：缺周期/天数报错，扣款日自动取 DDL 不报错；更新行缺周期/天数且库内为空 → 报错，缺扣款日 → 自动取生效 DDL；**续费行扣款日一律强制 = 生效 DDL**（行内显式值忽略，2026-09-03）；若历史数据存在不完整形态 → 不结算不提醒，详情内提示补全（防御性兜底）。
...
- **B21 · 自动续费卡的手动顺延闸门** → 详情操作区「修改」按钮对续费卡**置灰**（点击提示「自动续费卡不可修改：请先关闭自动续费（续费区块开关），再来编辑」）；「编辑」弹窗内终止日期同样**置灰** + 提示「自动续费卡不可直接顺延：请先在续费区块关闭自动续费，再来修改终止日期」；操作区**不设独立「顺延」按钮**（2026-09-02 裁定：顺延统一走编辑弹窗改终止日期）。
- **B22 · 静默的本周期终点** → 自动续费卡：下次结算顺延即恢复；非自动续费卡：DDL 被顺延或过期即恢复。**解除渠道不限**：手动顺延（PATCH / POST 覆盖）、结算追赶、导入推进周期日期——凡 `end_date` 实际发生变化即自动解除「本周期」静默（含扫描校正提前；重新静默是一次点击）。**静默必须可见、可逆（防数据黑洞）**：进站 alert 行内与清单名称行提供**三态图标点击循环**（提醒 → 本周期 → 永久 → 提醒；非续费卡两态；2026-09-03 裁定）——清单内仅静默态显示图标，alert 内常驻且点击不关弹窗、条目不重排；详情静默菜单保留（5.4 / 5.7）。
...
- **B27 · `auto_renew = true` 缺周期或扣款日**（2026-09-03 二次裁定）→ **新增行**：缺周期/天数报错，扣款日不报错（自动取 DDL）；**更新行**：缺周期/天数且库内为空 → 报错；缺扣款日 → 自动取生效 DDL；**续费行扣款日一律强制 = 生效 DDL，行内显式扣款日忽略**（3.6.3 扣款日规则；cards-db.md 7.5 / 4-B27 合并态校验同步修订）。
...
- **排序行**（2026-09-13 起三键折叠为**排序下拉框**，样式对照周期下拉 `.cd-select`，组件 `src/components/SortDropdown.jsx` 余额页共用）：默认「**按到期日排序**」（DDL 升序——最先到期的在前；DDL ≡ 扣款日使单键同时覆盖两种关注点，原"关键日"取近者定义已取消，2026-09-02）；备选「按名称排序」（拼音序）、「按更新时间排序」（按 `updated_at`，**默认最新在前**）。下拉里选不同项 = 切换维度（各自默认方向），**重选当前项 = 翻转方向**，↑/↓ 恒随文案尾随。已过期与次数用完永远沉底，不参与主排序（两种沉底颜色区分，5.4；沉底卡命中扣款窗口时照常进 alert——位置与提醒是两件事，4-B12）。
...
| 5 | 次数减一 / 清零（标记用完） | **半展开右滑**（仅次数类生效卡）：浅拉松手露出「减一」按钮（点击核销一次，面板回缩可连续操作）；**拉过行宽一半或快速右甩 = 清零**，蒙层提示「拉到底 · 清空次数」，整卡飞出后直接提交 | 无（可逆；减到 0 / 清零触发按卡型提示，5.8） |
...
- **结构**：遮罩（复用弹窗遮罩）+ 白卡（`min(400px, 100%)`，圆角 16，spring 入场）：
  - 标题「有 N 张卡需要注意」；
  - 卡条目列表：每条 = 色点（记录色）+ 卡名全称 + 原因标签（「剩 N 天」/「MM-DD 扣款」/双标签；沉底的次数用完卡以**独立小标签**「已用完」叠加，4-B12）+ 行内**单个静默图标按钮**（2026-09-03 修订，取代原双文字按钮）：
    - 图标随状态切换（提醒中 = remind.svg / 本周期 = remind-disable.svg / 永久 = close-remind.svg），点击循环到下一档（自动续费卡三态、非续费卡两态）；
    - **点击不关闭弹窗**：条目常驻——静默后带「本周期静默/永久静默」文字标注、提醒标签消失，可再点回来；**条目顺序在弹窗打开时冻结**，静默/恢复不重排、不沉底；
    - 点击条目本体 → 关闭 alert 并半展开该卡；
  - 底部主按钮「知道了」（= 忽略，关闭弹层）。
...
### 折叠态（2026-09-06 修订：右槽升级为 30px 天数环）

- 左 `◆ 卡名全称`，卡名右侧**能力旗标**（名称超长截断时旗标不消失；两个旗标都没有时不渲染容器，行尾不留空占位）：
  - **自动续费旗标**（money.svg；自动续费卡显示；随开关实时出现/消失，纯展示）；
  - **静默循环按钮**（remind-disable.svg / close-remind.svg，在续费旗标之右；**仅静默状态显示**——提醒中不显示，2026-09-03 裁定；点击循环：本周期 → 永久 → 解除，非续费卡点击即解除，每档有 notice）。
- 右侧**双槽信息行**：左槽「剩 N 次」次数胶囊（次数卡；用完换灰色「已用完」胶囊）+ 右槽 **30px 天数环**（DaysRing：环内数字 = 剩余天数或扣款倒计时，环旁缀「天」字；**天数恒为主信息**；扣款窗口内分子为扣款倒计时）。沉底卡的环照常渲染（灰态）；无天数语义时回退文字。
- **环色按提醒窗口判定（2026-09-06 裁定，与静默解耦——静默只是不弹提醒，环色照常）**：常态绿 → 到期窗口（DDL−15）内橙 → 扣款窗口（扣款日−7）内红；**特例灰态**：已过期与次数用完（弧与数字同步变灰）。四档色值规格见 `design-language.md` DaysRing 节。
- **沉底卡原因必须可辨**：**两种沉底颜色不同**——到期 = 失效灰 `#CDD3D9`（真正的失效），次数用完 = 弱化灰蓝 `#D9E2EC`（资格暂尽不是失效，token 见 6.5 / design-language.md）；沉底卡文字与菱形同步压灰、**图标 logo 保持原彩色**；已用完卡以灰色「已用完」胶囊标注文字原因。**例外**：沉底的次数用完卡若同时命中扣款窗口，追加独立小标签「MM-DD 扣款」（与 alert 一致，4-B12）——沉底不等于对钱失明。
- **过期卡默认被 5.1 开关收起**，不在折叠列表出现。

### 半展开态（点击折叠卡；同一常驻实例展开，带"从小变大"过渡）

- 余额同款宽卡样式；标题 20–30px 自适应缩放（档位与余额展开态一致，2026-09-06 裁定）、图标 20–40px；展示：卡名 + 旗标、日期区间（`2026.08.31 − 2026.11.29`，**不带"剩 XX 天"静态文案**——天数在右侧圆环）、续费摘要（`[续费中] 每月 · 09-14 扣款` 或「未开自动续费」）。
- 右侧：**65px 天数环**（环内标签「天剩余 / 天后扣款」，色调规则同折叠态；沉底卡灰态），环右下角贴 **28px 徽标按钮「展开修改」**（expand-down.svg，点击整环区域外仅按钮进详情）；环下方纵排「剩 N 次」胶囊与原因/扣款标签。右滑过程中该侧整块隐藏、改显提示蒙层。
- **滑动操作**（余额三色滑动语义；拖动中卡内容随剩余宽度 90/150px 两档平滑渐隐，不生硬滑出）：
  - **右滑**（仅**次数类生效卡**——带次数且未用完；沉底/过期/无次数卡无此手势）：浅拉松手 → 露出「**减一**」按钮（点击核销一次，面板回缩可连续滑动）；**拉过行宽一半或快速右甩 = 清零**——蒙层渐显「拉到底 · 清空次数」、越过半程点亮填充，整卡向右飞出 220ms 后直接提交，行以"已用完"灰态重新归位；
  - **左滑**：拉出「**删除**」面板 → 红色确认弹窗（4-B33）。
- 点「**展开修改**」按钮 → 进入全量详情；再点击半展开卡本体 → 收回折叠。

### 全量详情（「展开修改」）

- 头部固定 22px 图标 + 18px 卡名（**不参与缩放**），右端「收起」按钮（fold-up.svg）点击**回落半展开**（不直接收回折叠，保留中间层）。
- meta 行：有效期 `2025.09.20 − 2026.09.19 · 剩 22 天`。
- 区块：次数区块（`剩 N / 共 M 次`，自带开关 + 值编辑草稿流，5.7）、续费区块（开关 + 周期下拉 + 渐进扣款日 + 周期刷新次数 + 草稿流，5.7）、静默菜单（见下）、「修改」入口（**卡名** / 起始日 / 终止日期等业务字段，卡名改名受 B3 同名唯一性约束；**续费卡修改入口置灰**——点击提示先关闭续费，B21）。
- **续费信息不完整警示**：历史数据"续费开但缺扣款日/周期"时，详情顶部出现黄色警告条：「续费信息不完整：缺少扣款日或周期，不会结算也不会提醒——请在下方续费区块补全。」
- **静默菜单**：点开显示选项、**点击菜单外部自动收起**；**自动续费卡三选项**（不静默 / 本周期静默 / 静默），**非自动续费卡两选项**（不静默 / 静默——无周期概念）。
- **无"永久卡/无限期卡"标注**（概念已取消，1.3）。

### 状态与展示视觉

已过期 = **失效灰** `#CDD3D9` 沉底（真正的失效）；次数用完 = **弱化色沉底**（`#D9E2EC` 底 / `#5C6B7C` 字，浅灰蓝系——资格暂尽不是失效；token `--cd-usedup-card / --cd-usedup-text` 由 design-language.md 定义）；静默 = 卡名右侧静默图标（remind-disable / close-remind 两态区分，仅静默态显示）+ 正常色（静默不是终结态，不用灰）。

## 5.5 建卡弹窗（单条添加 · 一种卡 + 能力开关）

- **没有互斥卡型**：一种卡（期限卡基座）+ 两个能力开关，向导只是字段组合预设；创建后可随时在详情页补开或关闭任一能力（4-B31）。
- **基础字段**：**卡名**（单字段，全称直接录入，placeholder 示例「如：腾讯视频（商户名[-卡种（可选）]）」；与已有卡同名 → 预填其当前状态、提交覆盖，静默设置保留不动）、**图标选择器**（v3.2：可搜索、默认精选 11 个、「无」= 显式无图标；未手动指定 = 按卡名自动匹配，选择器显示「自动」标注）、**起始日期**（默认今天、可改历史，校验 ∈ [今天−2 年, 今天]，配置 A）、**终止日期** label 未开续费时为「终止日期（不填 = 最长 2 年）」（留空创建时物化为今天 + 配置 B；填写时校验 ≥ 起始日 且 ≤ 今天+2 年，配置 B）。**开启自动续费时该字段置灰**，label 变「终止日期（= 扣款日）」、值回显扣款日（DDL ≡ 扣款日，3.4.5）。
- **能力开关①次数**：开启时**剩余次数必填且 > 0**（不允许留空或 0）。**每周期次数不在本区块配置**——已收编为续费区块的「周期刷新次数」（2026-09-03 裁定，与 5.7 详情编辑同则）。
- **能力开关②自动续费**（与详情续费区块同则，2026-09-02 同步裁定）：
  ...
  - **周期刷新次数**（2026-09-03 裁定，次数能力开启时出现；与 5.7 详情同款）：下拉「无限 / 固定次数」+「每周期刷新」输入框；
    - **默认规则**：拨开续费开关且次数能力已开 → 默认**固定次数 = 剩余次数输入值**（开续费的直觉就是"每周期恢复到买的时候"）；同名预填已带每周期次数 → 保留预填值；
    - 选「无限」→ 清空数值 + 内联提示：「无限：次数不会随周期重置，将保持当前数值（适合由外部扫描校准的次卡）；若希望每周期恢复次数，请选固定次数」（不自动关闭次数能力——外部校准型次卡是合法形态）；
  - 提示文案：「先选周期再选扣款日；扣款日限本周期内，保存后开启，有效期与扣款日一致」；
  - 校验顺序：缺周期 →「请选择扣款周期」；固定天数非正整数 → 行级报错；固定次数模式数值非正整数 → 行级报错；缺扣款日 →「请选择扣款日」；超出周期限窗 → 提示窗口范围。
- **已过期卡上开关置灰**：DDL 填了过去日期（B4 允许录入已过期卡）时，自动续费开关置灰 + B23 文案。置灰拦的是"从关拨到开"；同名覆盖表单预填已开启的（过期 + 续费中的既有卡）保持可交互——**维持不是开启**。
- 校验实时化：DDL 与起始日区间、次数开启必填 > 0、周期刷新次数（固定模式）> 0、开续费必填周期 + 扣款日（同 4-B39 限窗）。
- 提交中「保存中…」双按钮禁用；失败弹窗内红色 notice、内容保留；成功走 notice（含窗口提示，4-B38），不弹 alert。

## 5.6 批量导入页

- 标题「**会员卡批量导入**」（2026-09-03 随标签更名）；骨架复用 `BalanceImport`：hero 说明文案 → 大 textarea（占位文案给出列格式示例）→ 解析预览 → 全宽提交按钮 → 结果 notice。**内容列宽 ≥ 视口 2/3**（2026-09-03 裁定，余额批量导入页同款；小屏退回 560px 列）。
- **无表头时的默认列序（9 列）**：卡名 / 起始日 / 终止日期 / 剩余次数 / 每周期次数 / 自动续费 / 扣款周期 / 合同天数 / 扣款日；表头可识别别名（如「商户」列不再识别，含商户列的旧粘贴会被优雅忽略该列）。
- **预览区分三组**：
  1. 新增 N 条（常规预览表；由默认值填充的字段标注「默认」；续费行扣款日强制 = 生效 DDL 时带「扣款日」默认标注）；
  2. **更新 N 条（默认展开——这是唯一的人工核对关卡）**：每行紧凑对照（字段 | 旧值 → 新值；未变化字段不列）；扫描判定过期的行标注去向（「将更新为过期」/「跳过（库内已过期）」/「新增过期记录」）；
  3. 错误 N 行（浅红块逐行报错，沿用现有样式；**解析错误与分类错误合并后统一按行号升序**，2026-09-03 修订）。续费行扣款日强制 = 生效 DDL（3.6.3 扣款日规则）——行内写的扣款日被忽略，diff 如实显示。
- **可回改**：预览里发现不对的值 → 直接改 textarea 原文 → 重新解析 → 再预览；扫描错误的修正入口就是粘贴文本本身。
- 提交可用条件：≥1 条有效且**零错误（解析 + 分类合并计数）**；提交中「提交中…」禁用；成功后 textarea 清空 + 成功 notice 汇报「新增 N / 更新 N / 跳过 N」。
...
- **自动续费开关**（iOS 式，复用 0 余额开关交互；**开关是续费的唯一入口**——拨关 = 关闭自动续费（3.4.4 唯一显式打断），拨开 = 开启草稿，操作区不另设续费按钮）：
  ...
  - **周期刷新次数（2026-09-03 裁定，次数能力开启时出现在续费区块内）**：下拉「无限 / 固定次数」+「每周期刷新」输入框，与 5.5 新增弹窗同款；**默认规则**：给从未配置每周期次数的卡开启续费 → 默认**固定次数 = 库内剩余次数**；已有配置从现值出发；选「无限」→ 清空数值 + 与 5.5 同款内联提示。关闭续费开关时 total_sessions 一并置空（2026-09-02 裁定），次数能力保留。
- **次数区块**：次数区块**自带开关**，与续费区块完全对称——开关在区块头部，**值操作与能力开关入口分离**（值操作 = 半展开右滑清零/减一 + 区块内值编辑；开关 = 能力的结构操作）：
  - **开启（事后补开）**：剩余次数必填且 > 0（与 5.5 创建时同一条规则）；补开后区块内露出值编辑（**仅剩余次数**——每周期次数已收编至续费区块的周期刷新次数，2026-09-03 裁定），**草稿流**（改动后出现保存/取消，保存才落库——引导用户确认，无悬空修改态）。
...
## 5.9 日历展示模式（2026-09-03 新增 · 飞书日程风格）

- **模式切换**：清单排序行右端分段控件「列表 ⇄ 日历」，默认列表；纯前端状态，两模式共用同一份 store 快照，零额外请求；不持久化，进站默认列表。
- **布局**：日历模式独立工具栏（`‹ YYYY年M月 ›` 翻页 + 「今天」回跳〔当前月置灰〕+ 卡片筛选 + 模式切换）；月视图网格以周一为首列；**日历不受列表列宽约束**（居中突破至最大 1040px，小屏填满视口）。
- **日程条**：每张卡一条 起始日 → 终止日 的跨天条，底色 = 该卡记录色（与列表同卡同色）；跨周折行、跨月断口画延续箭头；**条文字（卡名）只在本月首次出现的头部段显示**，续周只画色条，跨月后在新月头部重新显示。
- **今日与过去**：今天的日期数字 = 品牌蓝胶囊 + 整列淡底；**今日之前的段整段置灰**（以今天为界的硬分界双色渐变——单条日程上实现、不拆段）；已过期卡翻到历史月份即为灰色删除线条纹——不受列表"展示已过期"开关影响。
- **卡片筛选（多选下拉）**：**选中集语义，最多同时 5 张**——卡数 ≤5 默认全选，>5 默认不选（空态引导）；勾满后未选项禁勾 + 底部提示；面板内支持**卡名关键词搜索**与全选/清空（总数 >5 时全选禁用）；筛选状态在列表 ⇄ 日历切换间保留，不持久化。
- **交互**：点日程条 → 日历内弹出该卡**只读详情**（状态 / 有效期（含剩 N 天或已过期天数）/ 次数 / 自动续费 / 静默 五行展示，不允许修改——改卡走列表详情态）；弹窗实时反映数据，卡被删除自动消失。
- **明确不做（首版）**：周/日视图、拖拽改期、农历节假日、新建日程入口、筛选持久化、ics 导出（1.3）。
...
```
┌──────────────────────────────────────────┐
│ ██ 深蓝渐变 hero：眉题 MEMBERSHIP OVERVIEW │
│    字标 H1        · 退出登录 · 登录态胶囊  │
├──────────────────────────────────────────┤
│ [余额 12] [会员 8] [优惠券 4]             │ ← 分段控件（选中=品牌蓝；会员=原卡包）
│ [按到期日排序 ↑ ▾]      [列表|日历] │ ← 排序下拉 + 模式切换（2026-09-13 折叠）
│ [○ 展示已过期的卡片（3）]                 │ ← 过期筛选开关（默认关；0 余额开关同款）
│ ┌────────────────────────────────────┐ │
│ │◆ 爱奇艺-月会员 $        ◔3 天      │ │ ← 折叠卡（icon 主色渐变底；右槽 30px 天数环）
│ ├────────────────────────────────────┤ │
│ │◆ 超鹿-30次卡     剩 8 次  ◔22 天    │ │ ← 次卡双槽：左次数胶囊 + 右天数环
│ ├────────────────────────────────────┤ │
│ │◆ 美甲次卡 🔇     已用完 ◔ [09-14 扣款]│ │ ← 弱化色沉底（灰环）+ 独立小标签（4-B12）
│ └────────────────────────────────────┘ │
│              （ ⊕  ）                    │ ← FAB：单条添加 / 批量导入
└──────────────────────────────────────────┘
   内容列 560px 居中；卡包层叠 -16px 叠压；滚动条 gutter 稳定不挤压行宽
   已过期卡默认被开关收起；$ = money.svg 自动续费旗标
   ◔ = 30px 天数环（绿/橙/红/灰四档，2026-09-06；环内数字 + 环旁「天」字）
   🔇 = 静默图标（remind-disable/close-remind 两态，仅静默态显示；点击循环档位，2026-09-03）
```
...
└─────────────────────────────────────────────────────────┘
   ...
   [🔔]/[🔕] = 静默循环图标按钮（提醒中/静默态），点击切档：弹窗不关、
   条目不重排；静默后条目带「本周期静默/永久静默」文字标注（2026-09-03）
...
```
折叠（默认）                 半展开（点击折叠卡）              全量详情（点环角「展开修改」）
┌───────────────────────┐  ┌───────────────────────────┐  ┌────────────────────────────────┐
│◆ Tony-理发季卡       │  │ ◆ Tony-理发季卡             │  │ ◆ Tony-理发季卡   （点头部回落半展开）│
│ [🔕]    剩 8 次 ◔22天 │  │ 2026.08.31 − 2026.11.29     │  │ 有效期 2026.08.31 − 2026.11.29 · 剩 22 天 │
└───────────────────────┘  │ [续费中] 每月 · 09-14 扣款   ◔ │  │ 次数区块  [开关 ●] 剩8  [保存][取消] │
                            │                        ⊕↘ │  │ 续费区块  [开关 ●] 周期▾ 固定天数… 扣款日… │
                            │ 剩 8 次 · [09-14 扣款]      │  │ 周期刷新次数 [无限▾/固定次数▾]        │
                            └───────────────────────────┘  │            [保存][取消]               │
                                                            │ [静默▾(3/2选项)] [修改]               │
                                                            └────────────────────────────────┘
   折叠卡名右侧旗标：$ = money.svg 自动续费 / 🔇 静默图标仅静默态显示
   （remind-disable/close-remind 两态区分本周期·永久；点击循环档位，2026-09-03）
   ◔ = 天数环：折叠 30px、半展开 65px（环右下角 28px 徽标 = 展开修改入口）
   次数右滑仅"次数类生效卡"：浅拉 = 减一（面板回缩可连续点），拉到底/快甩 = 清零（飞出直提交）
   操作区无「顺延」「删除」按钮：顺延走「修改」弹窗；删除走半展开左滑
```
...
- Token、圆角阶梯、阴影表、动效曲线全部沿用 `design-language.md`；**沉底色 token**：已过期复用 `--bd-zero-card`（`#CDD3D9`），已用完专用 `--cd-usedup-card`（`#D9E2EC`）/ `--cd-usedup-text`（`#5C6B7C`）——这是本模块新增的两个颜色；提醒角标用品牌浅蓝底 + 深蓝字（信息语义）；
- **卡底渐变对齐余额体系（v3.2）**：icon 主色 → 白的 90° 渐变（`cardBgStyle`），名称行文字色按 WCAG 对比度自动选白/深，自动续费与静默旗标随对比度翻白（`--cd-flag-filter`）；无图标卡回退记录色板（青绿-紫罗兰系 17 色，id 哈希取色不变色）；沉底卡不套渐变（保持灰底），图标 logo 保持彩色不灰化；
- **金额不存在于本模块**（数据模型无金额字段，视觉层不得出现价格）；数字展示（天数/次数）用 `tabular-nums`，天数以右侧圆环为主信息（折叠 30px / 半展开 65px，四档色 RING_TONES）；
- 沉底卡原因必须可辨（灰色胶囊「已用完」/ 独立小标签），不允许只变灰（5.4）；**两种沉底颜色不同**：已过期 = 失效灰 `#CDD3D9`（真正的失效），已用完 = 弱化灰蓝 `#D9E2EC`（资格暂尽）；状态/提醒标签统一为**独立小标签**（「已用完」「MM-DD 扣款」「续费中」），不使用括号后缀拼接，样式复用提醒角标体系；
...
1. **库表与实现（已完成，本 v3.2 即最终结论）**：`supabase/cards.sql` v3（含旧表幂等迁移段：卡名合并、indeterminate 处理见 4-B36/B5）、`src/doc/cards-db.md` v3、第五章全部交互（三级展开、滑动操作、续费草稿流、排序与过期筛选、静默图标循环、日历展示模式）均已实现并通过验证（21 个领域用例、构建、lint）。
